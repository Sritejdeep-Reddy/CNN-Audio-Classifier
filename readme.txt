...
We have used evaluate and test function, passing data seperately for test(not in batch).

We have included the first model by default, if wanted can be replaced by second model right inside the test.py file. If the architecture is included the model.pt file can be changed and the model could be run. 
